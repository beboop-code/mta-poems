...
client.on("connect", () => {
  ...
  //client.publish("mta/poems", "Hello from p5.js!");
});
...
client.on("message", (topic, message) => {
    ...
    let newlines = mqttMessage.split(/\r?\n/); 
    x=random(20,150);
    y=random(120,350);
    let spaceBetweenLines=5;
    for(let i=0;i<newlines.length;i++){
        drawString(newlines[i], x,y +i*spaceBetweenLines , 0.2, i==newlines.length-1);
    }
